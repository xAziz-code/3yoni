...
local function handleMenuAction(action)
    print("Action received: " .. tostring(action))
...
AddEventHandler('macho:menuAction', function(action)
...
CreateThread(function()
    while true do
        ...
        if visible then
            ...
            if IsControlJustPressed(0, 176) then send({type="confirm"}) end
            if IsControlJustPressed(0, 177) then send({type="closeSub"}) end
        end
